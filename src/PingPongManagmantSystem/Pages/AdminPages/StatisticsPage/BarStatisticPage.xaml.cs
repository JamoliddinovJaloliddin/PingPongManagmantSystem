...
namespace PingPongManagmantSystem.Desktop.Pages.AdmiPages.StatisticsPage
{
    ...
    public partial class BarStatisticPage : Page
    {
        ...
        private async void Refresh_BarStatistic()
        {
            try
            {
                var statisticResult = await barStatisticService.GetAllBarStatistic("", new PaginationParams((int)GlobalVariable.Page, pageSize));
        ...
        private async void Search(object sender, TextChangedEventArgs e)
        {
            try
            {
                var statisticResult = await barStatisticService.GetAllBarStatistic(tb.Text.ToString().ToLower(), new PaginationParams((int)GlobalVariable.Page, pageSize));
                barStatisticDataGrid.ItemsSource = statisticResult;
            }
            catch
            {
